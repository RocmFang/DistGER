...
- <p align="justify"> The efficiency improvement and scalability of DistGER with more machines is shown in Fig 6, Sec. 6.3. </p>
...
- <p align="justify"> Thanks for pointing it out, we will modify it in the new version. </p>
...
- <p align="justify"> For the billion-edge graph Twitter, the current solutions either cannot be terminated in a reasonable time, or crash due to exhausting the available hardware resources. For instance, DistDGL does not terminate in 1 day, while KnightKing triggers out-of-memory during sampling. Although Pytorch-BigGraph can be done in 2173s on 8 machines, it can be observed that on 4 machines it takes even less time – 1767s, indicating that the hardware resources cannot be fully utilized due to poor scalability which is consistent with Fig. 6. Our proposed DistGER only needs 745s with better task-accuracy and scalability.</p>
