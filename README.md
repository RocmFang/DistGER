...
- Since one of the motivations of our work is that there is yet no end-to-end distributed system to support graph embedding via information-oriented random walks, the competitors we selected are state-of-the-art distributed graph embedding systems.
...
- HuGE+ is also a recent approach (IEEE TBD 2022), which is an extension of HuGE, it uses the same HuGE heuristic method to determine the walk length and number of walks per node, thus the efficiency difference is not much in HuGE and HuGE+. DistGER’s general API  is compatible to run HuGE+.
