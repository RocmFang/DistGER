...
vector<int> word_freq_block_ind; // Ma
...
#pragma omp parallel for num_threads(num_threads) reduction(+ \
    ...
    {
        table[a] = i; //  UnigramTable 中的元素是 000，1111，2222这样的
        if (a / (real)table_size > d1)
        {
            i++; // 一个单词的比例超过了，换下一个
            if (i >= vocab_size)
                i = vocab_size - 1; // table 末端可能全是最后一个单词
...
void my_SortVocab()
{
    ...
    //     // 删除低频词
    //     if((v_vocab[i]->cn < min_count)&&(i != 0))
    //     {
    //         cn_vocab[v_vocab[i]->id]=-1; // 在 cn_vocab 中标记 -1 代表被删去
    //         vocab_size--;
    //     }
    //     else{
    //         train_words += v_vocab[i]->cn;
    //     }
    // }
}
...
void my_LearnVocabFromTrainFile()
{
    ...
    // 以索引为序号在 词表中的位置，例如 1 在 词表的位置是 hash2new_sort[1]
    // 首先确定是什么 映射到什么，什么是输入，什么是输出
    // 把坐标当作 输入 ，x  f(x), 把映射结果填到坐标处。
...
ulonglong loadStream(FILE *fin, int *stream, const ulonglong total_words)
{
    ...
    stream[word_count] = 0; // set the last word as "</s>" ?? 为什么 0 是空格
    return word_count;
}
// assume v > 0
...
#pragma omp parallel num_threads(num_threads)
    {
        ...
        if (id == 0)
        {
            ...
            //同步块大小 单位 字节？？  这里的4 应该是一个词有4个hidden size 大小的数组需要同步
            int sync_chunk_size = message_size * 1024 * 1024 / (hidden_size * 4); // message size 是1024MG 也就是1GB，一个词向量是 hiddensize*4 B
                                                                                  // chunk size 就是一个 message size 能有多少 个词
...
#pragma omp atomic
            ...
            double start = omp_get_wtime(); // 单位 s   返回到过去某一点的时间，在某次运行时 某一点 不会变
            ...
            while (1)
            {
                ...
                
                {
                    ...
                    // 过了多少个需要同步的时间   同步计数
                    num_syncs++;
                    // 如果 活跃计算线程为0，则自己标记为不活跃进程
                    active_processes = (active_threads > 0 ? 1 : 0);

                    // 全局同步 全局活跃进程数 和 全局单词训练数
                    ...
                    // 需要同步的词表大小    min_sync_words = 1024  每次最少同步的单词数，
                    // 同步词表大小是逐渐增大的，按照同步次数的位数乘以 最小同步词数
                    ...
                    real progress = word_count_actual_global / (real)(iter * train_words + 1);
                     ...
                     //============= all sync ================
                    //  指定 全模型同步次数，然后根据处理进度，定期进行全模型同步，例如，如果 full_sync_times 等于4，就会在 20%,40%,60%，80% 等，进行全同步
                    if ((full_sync_times > 0) && (progress > (real)full_sync_count / (full_sync_times + 1) + 0.01f))
                    {
                        ...
                        full_sync_count++;
                        int sync_vocab_size = vocab_size; // 直接全词表同步
                        sync_block_size = vocab_size;

                        // ？？ 把需要同步的词表拆成 多个 sync_chunk(单位 word) 来同步 ？？ sync_chunk_size 的意义？
                        int num_rounds = sync_vocab_size / sync_chunk_size + ((sync_vocab_size % sync_chunk_size > 0) ? 1 : 0);
                        // 【每次都会同步到前面得高频词，中间穿插几次 全模型 同步，然后每次同步得词表范围变大，逐步同步到后面得低频词】
                        for (int r = 0; r < num_rounds; r++) // 进行同步   一直在同步高频词，一直在同步头部的高频词
                        {
                            int start = r * sync_chunk_size;
                            int sync_size = min(sync_chunk_size, sync_vocab_size - start);
                            // Wih Woh 这两个应该是 中心词矩阵 和 周围词矩阵  为什么是 MPI_SUM，因为后面在本地在做平均
                        ...
                        // 相当于本地做平均，因为之前同步时是直接把梯度相加求和
    #pragma simd
    #pragma vector aligned
                        for (int i = 0; i <sync_vocab_size * hidden_size; i++)
                        {
                            Wih[i] /= num_procs; // 同步过的梯度再按机器数做平均
                            Woh[i] /= num_procs;
                        }
                    }
                    //========== all sync =============
    ...
    #pragma vector aligned
                        ...
                        {
                            sync_block_in[i] /= num_procs; // 同步过的梯度再按机器数做平均
                ...
                while(sentence_length == 0)
                {
                    ...
                    while (true)
                    {
                        ...
                        if (origin_id == -1 || cur_words >= start_words + thread_words_num)
                            break;
                        w = hash2v_vocab[origin_id]; // 要把原始的路径映射到 v_vocab 中，因为 在 Wih 中也是按照 v_vocab 排序的，高频节点在前面
...
#pragma omp atomic
                ...
                next_random = next_random * (ulonglong)25214903917 + 11;
                int b = next_random % window; // 窗口中随机选一个起点？

                int num_inputs = 0; // 真正的窗口大小是 左右两个 Window 。
                // 窗口左右 收缩了
                for (int i = b; i < 2 * window + 1 - b; i++)
                {
                    if (i != window)
                    {
                        int c = sentence_position - window + i; // c 在句子中的位置
                        if (c < 0)
                            continue;
